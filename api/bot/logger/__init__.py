...
from api.bot.logger.loggerview import LoggerView
...
class Logger(DiscordFeaturesMixin):
    ...
    async def register_logger_views(self):
        sessions: list[dict] = await request('session/all/')
    ...
    async def session_over(self, channel_id: int):
        ...
        members = await request(f'session/{channel_id}/members')
        members_field = '└ ' + ', '.join(f'<@{member["id"]}>' for member in members)
        ...
        await request('leadership/', 'post', json=json)
        await self.events.session_update(channel_id=channel_id, end=now())
    ...
    async def update_leader(self, channel_id: int, leader_id: int):
        try:
            session = await self.get_session(channel_id)
            if not session:
                raise AttributeError(f'Trying to update session({channel_id}) leader but session isn\'t registered')
    ...
    async def update_embed_members(self, session_id: int):
        try:
            members: list[dict] = await request(f'session/{session_id}/members')
    ...
    async def update_activity_icon(self, channel_id: int, app_id: int):
        ...
        app_info = await request(f'activity/{app_id}/info')
        if not (session and app_info):
            return
        ...
        emoji = await request(f'activity/{app_id}/emoji')
        if emoji:
    ...
    async def log_member_join(self, user_id: int, channel_id: int):
        try:
            await request(f'session/{channel_id}/members/{user_id}', 'post')
