...
class LoggerView(discord.ui.View, BaseCogMixin):
    ...
    async def format_data(self, response: list[dict], header: str, col: str, activity_flag: bool = False) -> tuple[str,
        ...
        for row in response:
            ...
            if not activity_flag:
                ...
                activity = await request(f'/activity/{row["id"]}/info')
    ...
    async def leadership(self, interaction: discord.Interaction, _) -> None:
        ...
        leadership = await request(f'session/{interaction.message.id}/leadership')
    ...
    async def activities(self, interaction: discord.Interaction, _) -> None:
        ...
        activities = await request(f'session/activities/by_msg/{interaction.message.id}')
    ...
    async def prescence(self, interaction: discord.Interaction, _) -> None:
        ...
        prescence = await request(f'prescence/by_msg/{interaction.message.id}')
