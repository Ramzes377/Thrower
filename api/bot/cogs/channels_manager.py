...
from api.bot.logger import Logger
...
class ChannelsManager(DiscordFeaturesMixin):
    ...
    async def handle_created_channels(self):
        ...
        sessions = await request('session/unclosed/')
    ...
    async def on_presence_update(self, before: discord.Member, after: discord.Member):
        await self._logger.log_activity(before, after)
    ...
    async def on_guild_channel_update(self, before: discord.VoiceChannel, after: discord.VoiceChannel):
        ...
        if need_save and before.name != after.name:
            session = await request(f'session/{after.id}')
            await request(f'user/{session["leader_id"]}', 'put', json={'default_sess_name': after.name})
    ...
    async def user_try_create_channel(self, user: discord.Member, user_channel: discord.VoiceChannel):
        ...
        await self._logger.session_begin(user.id, user_channel)  # send session message
    ...
    async def join_to_foreign(self, user: discord.Member,
        ...
        if user_quit_channel:
            await self._logger.log_member_abandon(user.id, prev_channel.id)

        if user_join_channel:
            await self._logger.log_member_join(user.id, cur_channel.id)
    ...
    async def transfer_channel(self, user: discord.Member, channel: discord.VoiceChannel):
        try:
            ...
            await self._logger.log_activity(None, new_leader)
            await self._logger.update_leader(channel.id, new_leader.id)
    ...
    async def end_session(self, channel: discord.VoiceChannel):
        await self._logger.session_over(channel.id)
