...
class Channels_manager(Base_COG):
    ...
    async def on_ready(self):
        ...
        await self._delete_unused_roles()
        await self._delete_removed_emoji()

        print(f'{type(self).__name__} starts')

    async def _delete_unused_roles(self):
        ...
        async with self.get_connection() as cur:
            for role in roles:
                ...
                if await cur.fetchone() and len(role.members) < 2 and (now - role.created_at).days > 60: #gamerole exist
                    ...
                    if app_id:
                        ...
                        if emoji_id:
                            await cur.execute(f"DELETE FROM CreatedEmoji WHERE emoji_id = {emoji_id[0]}")
                    await cur.execute(f"DELETE FROM CreatedRoles WHERE role_id = {role.id}")
                    await role.delete()
    ...
    async def _link_gamerole_with_user(self, user):
        ...
        async with self.get_connection() as cur:
            ...
            if created_role:  # role already exist
                ...
                if not role in user.roles:  # check user have these role
    ...
    async def _start_session_message(self, creator, channel):
        ...
        async with self.get_connection() as cur:
            ...
            embed_obj.add_field(name='├ Сессия активна', value=f'├ [ВАЖНО!]({choice(urls)})', inline = False)
            embed_obj.add_field(name=f'├ Время начала', value=f'└ **`{text_time}`**', inline=True)
            embed_obj.add_field(name='Текущий лидер', value=f'{creator.mention}', inline=True)
    ...
    async def _end_session_message(self, channel):
        ...
        if sess_duration.seconds > 300:
            ...
            embed_obj.add_field(name=f'├ Время начала', value=f'├ **`{time_formatter(start_time)}`**', inline=True)
    ...
    async def _transfer_channel(self, user, channel):
        ...
        permissions = {user: default_role_rights, new_leader: leader_role_rights}
        try:
        ...
        async with self.get_connection() as cur:
            ...
            *_, message_id = session_info
            try:
                msg = await self.bot.logger_channel.fetch_message(message_id)
                emb = msg.embeds[0]
                emb.set_field_at(2, name = 'Текущий лидер', value = new_leader.mention)
