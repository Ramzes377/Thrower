...
from bot.mixins import BaseCogMixin, commands


def message_belongs_bot(author_id: int) -> bool:
    return any(author_id == bot_id for bot_id in Config.BOT_IDS)
...
class FloodManager(BaseCogMixin):
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        if author == self.bot.user or message.channel.id == Config.COMMAND_ID:
            return
