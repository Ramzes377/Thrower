...
def user_sessions(user_id: UserID = Depends(),
...
def user_activities(user_id: MemberID = Depends(),
                    service: SrvUser = Depends()):
...
def user_activities_durations(user_id: MemberID = Depends(),
                              service: SrvUser = Depends()):
    return service.durations(user_id)


@router.get('/{user_id}/activities/duration/{role_id}',
