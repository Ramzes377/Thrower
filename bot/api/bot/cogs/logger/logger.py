...
class LoggerHelpers:
    ...
    async def restore_log_session_message(self,
        ...
        for session in await self.db.get_all_sessions(from_date, to_date):
            ...
            embed = (
                discord.Embed(title=f"Сеанс {name} окончен!",
                              color=discord.Color.red())
                .add_field(name=f'├ Время начала',
                           value=f'├ **`{self.fmt(begin)}`**')
                .add_field(name='Время окончания',
                           value=f'**`{self.fmt(end)}`**')
                .add_field(name='├ Продолжительность', value=duration_field,
                           inline=False)
                .add_field(name='├ Участники', value=members_field,
                           inline=False)
                .set_footer(text=creator.display_name + " - Создатель сессии",
...
class LoggerEventHandlers(DiscordFeaturesMixin, LoggerHelpers):
    ...
    async def update_embed_members(self, session_id: int):
        ...
        embed.set_field_at(2, name='├ Участники',
                           value='└ ' + ', '.join(
                               f'<@{member["id"]}>' for member in members),
                           inline=False)
    ...
    async def _member_join_channel(self, member_id: int, session_id: int):
        ...
        try:
            ...
            await self.wait_for_session_created(
                session_id)  # session still not created
            await _add_member()

        await self.db.prescence_update(channel_id=session_id,
                                       member_id=member_id,
                                       begin=now())

    async def _member_abandon_channel(self, member_id: int, channel_id: int):
        await self.db.prescence_update(member_id=member_id,
...
class Logger(LoggerEventHandlers):
    ...
    async def on_session_begin(
        ...
        embed = (
            discord.Embed(title=f"Активен сеанс: {name}",
                          color=discord.Color.green())
            .add_field(name=f'├ Время начала',
                       value=f'├ **`{self.fmt(begin)}`**')
            .add_field(name='Текущий лидер', value=creator.mention)
            .add_field(name='├ Участники', value='└ ' + f'<@{creator.id}>',
                       inline=False)
            .set_footer(text=creator.display_name + " - Создатель сессии",
                        icon_url=creator.display_avatar)
        )

        msg = await self.bot.channel.logger.send(embed=embed,
                                                 view=LoggerView(self.bot,
    ...
    async def on_session_over(self, channel_id: int):
        ...
        embed = (
            discord.Embed(title=f"Сеанс {sess_name} окончен!",
                          color=discord.Color.red())
            .add_field(name=f'├ Время начала',
                       value=f'├ **`{self.fmt(begin)}`**')
            ...
            .set_footer(text=msg.embeds[0].footer.text,
    ...
    @commands.Cog.listener()
    async def on_activity(self, before: discord.Member | None,
        ...
        if after_app_id and not self.cache.get(after):
            # for some reason discord rest calling event on_prescence_update
            # twice with same data
            self.cache[after] = dt.isoformat()
            await self.db.member_activity(member_id=after.id, id=after_app_id,
                                          begin=dt, end=None)
        ...
        if before_app_id and not self.cache.get(before):
            ...
            await self.db.member_activity(member_id=before.id, id=before_app_id,
                                          end=dt)
    ...
    async def on_leader_change(self, channel_id: int, leader: discord.Member):
        ...
        await msg.edit(embed=embed)
        await self.db.update_leader(channel_id=channel_id, member_id=leader.id,
                                    begin=now())
